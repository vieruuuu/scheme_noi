...
proc render*(data: openArray[string]): string =
  ...
  for keyCodeOld in data:
    if keyCodeOld == "":
      continue

    var keyUp: bool = false
    var caps: int = 0
    var keyCode: string = keyCodeOld

    if keyCode[0] == '<':
      keyUp = true
      keyCode = keyCode[1..<keyCode.len]
    ...
    finalData.add(
      "<span class=\"tag " & color & "\">" & keys[keyCode.parseInt] & "</span>"
    )


  finalData.add "</div>"

  result = file.replace("$dataProcessed", finalData)
  result = result.replace("$data", data.join(" "))
  result = result.replace("$id", id("keylog"))
